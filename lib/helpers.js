...
console.log(locale);
    ...
    function intlBlock (options) {
        ...
        /* jshint proto:true */
        data.__proto__ = options.data;

        if (hash.locale) {
            data.locale = hash.locale;
        }
    ...
    */
    function intlMessage (str, options) {
        ...
        var hash = (options && options.hash) || {},
            ...
            locale = hash.locale || data.locale;

        return new IntlMessageFormat(str, locale, formatOptions).format(formatOptions);
    }
    ...
    */
    function intlNumber (num, options) {
        ...
        var hash = (options && options.hash) || {},
            data = (options && options.data) || {},
            formatOptions = hash.format && _numberFormats[hash.format] ||
                            extend({ currency: data.currency }, hash),
            locale = hash.locale || data.locale;
    ...
    */
    function intlDate (stamp, options) {
        ...
        return new Intl.DateTimeFormat(locale, formatOptions).format(stamp);
    }
    ...
    function intl (val, options) {
        ...
        if (!val) {
            if (options.hash.token) {
                val = options.data;
                token = options.hash.token.split('.');

                for (i = 0, len = token.length; i < len; i++) {
                    if (typeof val[token[i]] === 'undefined') {
                        throw new ReferenceError('Could not find path ' + options.hash.token + ' in ' + val + ' at ' + token[i]);
                    }
